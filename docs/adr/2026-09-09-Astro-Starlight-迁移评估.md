...
Starlight 在运行时架构、默认主题观感与搜索方案上具备客观优势，但当前项目处于跨端改造阶段，迁移不产生跨端收益，且主要成本集中在国际化与插件链重建。**建议暂不迁移**，待 Docusaurus v4 发布后结合其改进幅度重新评估。

若以「站点观感与性能」为首要目标而非跨端，则当前是内容结构最简单、自定义组件最少的窗口期，属迁移成本的低点。
