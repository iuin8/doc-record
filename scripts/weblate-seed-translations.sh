...
# 部分 Weblate 实例会拒绝创建组件。本脚本在源语言目录下逐个一级子目录放置一个
# 与源文同名、内容为空的译文文件，使掩码至少有一个匹配项。组件创建成功、语言启动
# 之后，Weblate 会按模板为其余源文件生成译文文件，种子文件即可删除。
#
# 用法：
...
#
# 可用环境变量：
#   SOURCE_DIR     源文目录，默认 zh-cn
#   CONTENT_DIRS   源目录下的子目录清单，默认与组件脚本一致
...
for dir in ${CONTENT_DIRS}; do
  ...
  mapfile -t sources < <(
    find "src/content/docs/${SOURCE_DIR}/${dir}" -name '*.md' -type f | sort | head -n "${SEED_COUNT}"
  )
  for src in "${sources[@]}"; do
    target="src/content/docs/${LANG_CODE}/${src#src/content/docs/${SOURCE_DIR}/}"
