...
> **当前状态：已暂缓（2026-09-14）。**
> 「单语言译文模版语言文件」字段在网页端与 REST API 两处均按单个实体文件校验，
> 而本仓库源文分散在 368 个文件中，该字段只能填 glob，一律被判为「文件不存在」。
> 唯一可行解是把根语言内容迁到真实语言目录，需改动全站 URL 结构，
> 成本高于当前收益，故暂缓。受阻分析与恢复条件见
> 《2026-09-10-翻译平台迁移至 Weblate》第 13 节。
>
> 本文档与 `scripts/` 下的脚本保持可用，满足恢复条件后可直接继续执行。

本目录提供 Weblate 部署所需的配置与说明，用于承接站点内容的翻译工作流。
决策背景见《2026-09-10-翻译平台迁移至 Weblate》。

**承载方式为 Hosted Weblate 的 Libre 计划**，步骤见第 0 节。
