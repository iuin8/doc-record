...
2. Health check parameter：Health Check command can use some parameters to configure intervals, timeout and return charts. Common health check parameters include：
   ...
   - `--retrieveves`：specialties a number of confidential failures to mark the service as unhealthy, default to 3.
...
In the above example, the flask service lies on the re-server service. He flask service's health check command is using the `curl` command to check if `http://localhost:50000` is accessible. We flask service will only start if the re-server service is healthy.
