# (frps selfhost, version) Can you access the entire network with a container? Use the container to connect to restricted networks: frp+ssh portfolio images, and shutley to open the network

> Use Container for Restricted Networks: frp+ssh Portfolio Imaging, and scroll to enable all inner network services (including k8s) in any environment (webpage and criminal access)
...
\- to install many bad vpn software, vpn used by different companies, vpn may be different, Some may focus like rogue software,various restrictions
...
> This is most complete where to connect, followed by simplified configuration, and advanced applications

## Upload a mirror to Aliyun to simplify the configuration of the launch

```bash
# Login to
docker logo --username=xxx@qqqq.com registry.cn-hangzhou.aliyuncs. om
...
Simplified only dock-compose, if `authorized_keys` is not integrated into Dockerfile, then mount the configuration is required.
...
There we use the little tool to forward traffic proxies to containers through ssh, so that we can access the corresponding web pages on the container side as we visit the local area network (LAN)
...
# proxy traffic
sshuttle --dns --auto-hosts --auto-nets-D -r cpolar.internet.company 10.0/24
...
- Note: Three conferences are mounted here
  ...
  - The second and third sessions are the meetings used by containers in name
