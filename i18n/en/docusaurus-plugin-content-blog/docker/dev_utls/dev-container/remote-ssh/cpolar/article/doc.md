...
> Use Container for Restricted Networks: cpolar+ssh Portfolios and scroll to enable all inner network services (including k8s) in any environment (webpages and criminal access)
...
A few points are usually identified at this time:
...
- Write a polar profile to mount
  ...
  - More tunnels need to be configured, then mount them out, then enabling the base process
...
ssh-copy-id polar. nternet.company -i ~/.ssh/id_ed25519
# and then log in
...
There we use the little tool to forward traffic proxies to containers via ssh, so that we want to access the corresponding web page on the container side of the local area network
...
# proxy traffic
sshuttle --sudoers-user fa --dns auto-hosts --auto-not-D -r cpolar.internet.company 10.0/24
```
...
- Next objective
  ...
  - The Intranet penetes into an open source `frp`, of course, it needs to have its own public server to plot the server
