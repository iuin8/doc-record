...
1. Send HTTP request： using curl
   ...
   curl-I <URL>
   ```
   this will send a HEAD request to the specified URL and return HTTP response header. If the connection successes, you will see the HTTP response code and other relevant information.
...
Note that these orders are only available for checking if the connection is successfully established, not for actual data transfer.
...
3. [Monitoring - Health check of web page using curl - Unix & Linux Stack Exchange change](https://unix.stackexchange.com/questions/84814/health-check-of-web-page-using-curl)
