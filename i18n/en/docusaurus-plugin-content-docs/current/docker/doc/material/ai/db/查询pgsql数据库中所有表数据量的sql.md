...
Method 1：uses pg_class table

```sql
SELECT relname AS table_name, relics AS row_count
FROM pg_class
WHERE relkind = 'r'
...
Method II：uses pg_stat_user_tables view
...
1. [postgresql queries the number of all table data under the current schema - Program ape friend 666 - Blog Gard] (https://www.cnblogs.com/chenghu/p/17201210.html)
