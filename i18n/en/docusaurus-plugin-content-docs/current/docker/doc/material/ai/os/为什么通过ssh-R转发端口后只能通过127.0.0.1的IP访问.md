# Why is it only accessible via an IP of 127.0.0.0.0.0. After passing through ssh -R to a port and cannot be accessed via an Extranet IP? What can be done to access via an Extranet IP?

By default, the SSH server will only list to the relationship port on the local address (127. This is for security reasons and to prevent unauthorized visits.
...
1. Modify configuration file：to open SSH server on SSH (usally /etc/ssh/sshd_config), find and modify the following line：
   ...
   This will allow the SSH server to list all imported ports on the network interface, not just local loop addresses.
...
3. Using an Extranet IP to access：on your client, you can now access exported ports via an Extranet IP using the SSH client. or example, if you forward ports to the SSH server 2222 and the SSH server has an extranet IP of 203. 1.113.1, you can connect to forward： using the following order.
...
Please note that administrators permissions may be required to modify the profile of the SSH server. n addition, Opening the forward port of the SSH server may entail security risks and ensure that only trusted hosts are allowed access to advance reports.
...
3. [Methodod of connecting inner hosts via ssh (firewall host) - memphise - log garden](https://www.cnblogs.com/memphise/articles/6472207.html)
...
Based on the search results, the "failed to start ssh. ervice: Unit not found" error may be due to using errong orders or different version of the system. How are several possible ways to solve this problem with：
