...
```shell
[root@localhost ~]# systemctl start rpin #start rpc service
[root@localhost ~]# systemctl enabling rind #set start
[root@localhost ~]# systemctl start nfs-security -server      
#startfs-server service and nfs secure transmission service (nfs-security server can not be needed)
[root@localhost ~]# systemctl enable nfs-server nfs-secure-secure-secure-server
