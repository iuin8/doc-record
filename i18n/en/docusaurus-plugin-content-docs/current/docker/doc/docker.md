...
```shell script
# Use --net=host (server.serviceName. etwork_mode=host) network_mode: "host", install of -p
...
systemctl daemonMaretoad # Refresh service
systemctl start docker # Restart docker
docker start registration # start registration # start registration # start registration
...
docker run --rm --name nmap securecodebox/nmap sh -c "nmap --p 92010. 7.0.24 | grep -C 5 open"
...
[参考地址2](https://github.com/influxdata/sandbox/issues/79) (PS: what to be used)
