...
## The jenkins instance look like to have been offline

    `````
    ````
    ``
    Installation of the plugin is a remoder of the page you offline and don't move.
    Then open a new tab, enter the URL `http://192.168.211.103:8080/jenkins/pluginManager/advanced`. 
    There is the bottom of a [upgrade site] and replace the link with https, http://updates.jenkins.io/update-center.json. 
    This close jenkins in the service list, then tomcat rebooks so that you can connect to
    ``
    ````
    `````
...
- -Dmaven.wagon.http.ssl.insecurity=true - Dmaven.wagon. http://www.ssl.allow=true - Dmaven.wagon. http://www.ssl.ignee.validity.data=true not used
