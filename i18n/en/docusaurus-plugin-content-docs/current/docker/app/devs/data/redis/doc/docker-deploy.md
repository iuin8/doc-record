...
docker create -name re-no-node-2 --net host -v /data/redis-data/node2:/data redis:5. 5 --cluster-enabled yes --cluster-config-file nodes-2.conf -port 6380
...
docker create --name redis-node6 --net host -v /data/read-data/node6:/data dis:5.0.5 -cluster-enabled --cluster-config-file node-6.conf -port 634
```
...
```shell
docker start remdis-node1 remdis-node2 remdis-node3 remdis-node4 remdis-node5 redis-node6
```
...
docker exec -it remdis-node1 /bin/bash
# Formation of clusters, 10. 11.55.4 as the current physical machine ipaddress
redis-cli --cluster create 10.211.55.4:6379 10.211. 5.4:632 11.55.4:632 10.211.55.4:6383 10.211.55.4:6384 --cluster-replas 1
...
```shell
# Manually add nodes
redis-cli --cluster add-node 10.211.55.4:6383 10.211.55.4:6379-cluster-sleve-cluster-master-master-id b0c32b1dae9e7f7f4b744354 c59bfcaa46f30a

redis-cli --cluster add-node 10.211.55.4:6379-cluster - cluster-master-id 111de8ed572585 cef5c4b5225ecb15a582e
```
