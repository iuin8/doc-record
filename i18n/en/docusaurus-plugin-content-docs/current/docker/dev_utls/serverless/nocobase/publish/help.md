# start-phase help document

<description>

Rapid employment of a job function to Ariun function jurisdiction

</description>

<table>

## Pre-preparation

With this item, it is recommended that you have the following product missions / policy：

| Services/operations  | Function Calcumulation                                 |
| -------------------- | ------------------------------------------------------ |
| Permissions/Policies | AliyunFullAccess<br/>AliyunContainerRegistryFullAccess |

</table>

<codepre id="codepre">

# Code & Preview

- [ :smiley_cat:  源代码](https://github.com/183461750/nocobase)

</codepre>

<deploy>

## Employment & Experience

<appcenter>

- :fire:  通过 [Serverless 应用中心](https://fcnext.console.aliyun.com/applications/create?template=start-fc-custom-container-websocket-nodejs14) ，
  [![Deploy with Severless Devs](https://img.alicdn.com/imgextra/i1/O1CN01w5RFbX1v45s8TIXPz_!!6000000006118-55-tps-95-28.svg)](https://fcnext.console.aliyun.com/applications/create?template=start-fc-custom-container-websocket-nodejs14)  该应用。

</appcenter>

- Upload： by [Serverless Devs Cli](https://www.serverless-devs.com/serverless-devs/install)
  - [Install Serverless Devs Cli Developer Tool](https://www.serverless-devs.com/serverless-devs/install) and conduct[授权信息配置](https://www.serverless-devs.com/fc/config);
  - Initialize project：`s init start-obase -d start-nobase`
  - Enter the project and upload：`cd start-base && s delete-y`

</deploy>

<appdetail id="flushContent">

# App Details

This app is used only for learning and reference purposes and you can redevelop and object your business logic on the basis of this project

</appdetail>
