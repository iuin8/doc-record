...
# config.yaml
wget -O config.example.yaml https://raw.githubusercontent.com/canber/WrenAI/0.13.2/docker/config. xample.yaml &&&\s
mkdir -p ~/.wrenai && cp config.example.yaml ~/.wrenai/config.yaml
# .env
wget -O .env. xss-sample https://raw.githubusercontent.com/canner/WrenAI/0.13.2/docker/.env.example &&\
...
> Record first, don't work, configuration too troubleshooter
