...
## /etc/etc/etc/etc/etc/etc/etcd. nv (this file may need to be created manually) (file in the current document directory)
...
sudo install minikube-linux-amd64 /usr/local/bin/minikbe
# please note that this method will continue to run minikebe with root, It may pose security risks because the "docker" driver should not be used as root.
minikbe start-force
# Dashboard
minikuku dashboard
```
...
Uninstall #
kubectl -f https://addons. uboard.cn/kuboard-v3.yaml
  # Cleanup legacy
