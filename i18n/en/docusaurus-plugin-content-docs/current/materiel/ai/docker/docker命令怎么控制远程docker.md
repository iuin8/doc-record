...
First, you need to confirm that the Docker daemon on the remote server is running. We can connect to the remote server via SSH and execute the following order to check：
...
Once this is done, you can run Docker orders on your local machine through `docker -H tcp://localhost:2376 ps`, which in fact will be forthcoming, via SSH tunnels to a remote Docker daemon.
