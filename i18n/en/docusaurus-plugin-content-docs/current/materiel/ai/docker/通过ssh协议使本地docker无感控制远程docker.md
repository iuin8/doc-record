# Preparing Docker SSH environment... How to prepare for the environment? What's the reason ? How I do I do it to use ssh's way to command a local docker to control the inner web docker via a docker
...
3. **Set SSH Prox**: IDEA uses an internal tool named `idea-docker-ssh` to forward requests from the local Docker client to the docker service of the remote via the SSH tunnel.
