...
4. Sign up submodules to the `.git/config` file of the main project to facilitate subsequent sub-module operations such as updating, push and more.
   In short, the `--recurse-submodules` function is to ensure that while the main project is cloned, All the submodules on which it relays are closed and set up so that developers can start working directly without directly initiating and updating submodules.
