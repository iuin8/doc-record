...
This orders you to get information about the process in which the `RSS` column shows the physical memory of the process (in KB).

**MethodII：uses the `top` command**

The `top` command is a dynamic system performance monitoring tool.

In the `top` interface, the `P` key can be sorted by CPU usage and the `M` key can be sorted by memory usage to process. In the process you are the following and see how they are using your memory.
