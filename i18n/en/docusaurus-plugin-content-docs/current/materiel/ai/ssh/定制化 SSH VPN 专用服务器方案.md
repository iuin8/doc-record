...
Match User vpn-tunnel
   ...
   ForceCommand /bin/false# its execution of any community
   Authorization Methods publickey
   PermitRoot Login no
   X11Forwarding no
...
1. **Get the server private keys**：  
   distressed the server-generated `/etc/ssh/vpn-server-key` private key to clients, save as `~/.ssh/vpn-default-key`
...
| **Two-factor authoration** | Integrated Google Authenticator imperfections dynamic passwords (need to modify my SSH PAM configuration) |
...
| Update Requirements | Key changes need to be signed       | No client needs to be updated                 |
