...
This is because dbus-daemon failed to start. The real system is not unusable. Set your CMD or entrypoint to /usr/sbin/init.Services like dbus will be automatically activated.
Then you can use systemctl. Order below：
docker run --primed -ti -e "container=docker" -v /sys/fs/cgroup:/fs/cgroup centos/usr/sbin/init
...
```bash
# Login to docker containing command reporting the following errors. $ `systemctl status sshd`
# pam_selinux(sshd:session): Error ending edit message.
# failed to create session access denied

# at the same time the following message can be seen in the login interface. $ `ssh -p 333 127.0.0.0.0.1`
# Root@127.0.0. ''s password: 
# Last login: Fri Oct 105:50:17 2024 from gateway
# /bin/bash: Permission denied
# connection to 127.

# Question reason: Starting SELinux, using makees the system safe but disrupts the operation of system files, creating an anomaly.
