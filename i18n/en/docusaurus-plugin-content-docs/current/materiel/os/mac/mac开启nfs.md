...
5. **Check the NFS shared directory**：uses the `showmount-e` command to see if the shared directory is properly configured. f The orders return the shared directory list, indicating that the NFS service has been successfully configured.
