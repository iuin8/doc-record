...
- 🌐 Dynamic domain parsing：will `<Any IP>.nip.io` automatically corresponded to IP
- 🚀 Zero configuration using：does not need to install customers or configuration DNS servers
- :wchildencard supports：for multi-tier subdomain dynamic resolution (e.g. `app.10.01.nip.io`)
...
to access the following informed domains： directly in browser or app

```bash
IPv4 Format
http://your-app.192-168-1-100.nip.io parto 192.168.1.1.00
...
```bash
Clone item
git block https://github.com/exentriquess/nip.io.git
