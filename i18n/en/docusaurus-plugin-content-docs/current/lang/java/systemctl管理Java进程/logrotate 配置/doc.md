...
| `root 30`              | Keep 30 adaptation logs (after which the gold is automatically deleted to avoid full disk)                         |
| `size 100M`            | Force conversion when a single log file reaches 100 MB (predefault ultra-large log files from being open)          |
...
| `postrotate`           | Script：executed after adaptation concerns the `USR1` sign to the Javatar process and notify the app to witness to the new log file    |

### Key concerns (Avoid loss of logs)

- **JavaScript application log witnesses support**：  
  Most JavaScript frames (e.g. Logback and Log4j2) support the `USR1` sign right log loading without restoring the app. f The app is not supported, use `copytruncate` (but may lose a small number of logs)：
  ```ini
...
- **Conce**：  
  `create` is required to match the app running (\`. , Otherwise the app may not have permission to write to the new log file.

- **Test configuration validity**：  
  Config to perform the following orders (no actual rotation, output execution plan only)：
  ```bash
  sudo logo -d /etc/logrotate.d/support-service
  ```
  Trigger a turn (for validation)：
  ```bash
