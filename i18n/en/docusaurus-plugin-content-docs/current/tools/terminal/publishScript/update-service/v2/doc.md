...
# and then log in to
ssh xxx.dev.iuin via sshshshshshshshshshopop.
```

```bash
# Updated environmental variable
remote service address in `base.sh` file based on project: REMOTE_SERVER="x.dev.iuin"
prefix for remote service directory: REMOTE_BASE_DIR="/data/xxxxx"
```
