...
# Another pleasant way is to use uv runn with --with [dependency] options, Follow you to run orders such as vlm serve without creating environment：
...
vllm serve Qwen/Qwen2.5-1.5B-1.5B-1.5B-1.5B-1.5B-1.5B-1.5B-1.5B-1.5B-Instruct -device cpu --enforc_eager
```
