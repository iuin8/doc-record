...
```bash
# Delete question Pod (Kubernetes will be autoronstructed)
kubectl delete pod <pod-name> -n <namespace>
```

## Problem

Bitnami PostgreSQL rors run by default using UID 1001 and GID 1001 non-root install of root)

```bash
# Parent directory to witness to NFS shared directories
cd /data/k8s/volume/postcongresql/

# View current permissions (confirmed as root:root)
dirls-Old postprogresql-data
